...
2. **Codex** (`/codex`) — searchable item data.
...
Preferences (platform, locale, panel layout, trackables) stay in the browser. The hub reads public Warframe worldstate/item APIs; it does not log into your Warframe account.
...
Human-readable release notes are published on **[GitHub Releases](https://github.com/WFCD/warframe-hub/releases)** (generated by semantic-release from Conventional Commits — not raw `git log`). Upgrade by pulling a newer GHCR tag or deploying the tagged commit. If a release fixes a publicly known vulnerability with a CVE (or similar), that fix is called out in the release notes when applicable.
