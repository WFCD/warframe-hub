<template>
  <div
    :class="
      mutableVariant +
        ' text-white rounded-full inline-block py-0 px-3 justify-end'
    "
  >
    {{ disp }}
  </div>
...
export default {
  ...
  methods: {
    ...
    formatTimer(diff) {
      ...
      [[86400000, 'd'], [3600000, 'h'], [60000, 'm'], [1000, 's']].forEach(
        ([unit, suffix]) => {
    ...
    onBadgeUpdate() {
      ...
      if (!this.counter) {
        ...
        if (!this.endtime) {
          ...
          this.mutableVariant = 'bg-blue-500';
          this.disp = `Starts in: ${this.formatDurationShort(
            durationactivate
          )}`;
        } else if (diff < 0) {
          this.mutableVariant = 'bg-blue-500';
          ...
          if (diff < 600000) {
            // 0 min to 10 min
            this.mutableVariant = 'bg-red-500';
          } else if (diff < 1800000) {
            // 10 min to 30 min
            this.mutableVariant = 'bg-yellow-500';
          } else if (diff > 1800000) {
            // 30 min to 1 hour
            this.mutableVariant = 'bg-green-500';
          } else {
            this.mutableVariant = 'bg-blue-500';
          }
